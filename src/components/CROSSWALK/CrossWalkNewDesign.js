...
const EditRecordAndRefreshFromAddNewRecord = async (_EmployeeID,_Position) =>{
...
const AddCrossWalkPositionAndRefresh = async (_EmployeeID,_Position,_SchoolName,e) =>{
...
function CellFormatterEditEmpDetails(cell, row) {
  return (
      <div>
          <Pencil onClick={(e) => EditRecordAndRefreshFromAddNewRecord(row.EmployeeID,row.Position)} />
      </div>
  );
}
...
function CellFormatterAddPosition(cell, row) {
  if( row.CrossWalked === 'YES') {
    ...
    return (
      ...
      <div>
          <PlusCircleFill onClick={(e) => AddCrossWalkPositionAndRefresh(row.EmployeeID,row.Role,row.SchoolName,e)} />
      </div>
  );
  }
}
...
export default connect(mapStateToProps,{fetchCrosswalkEntryData,
                                        ...
                                        fetchEmployeeData,
                                        fetchEmployeeIDSBySchoolName,
                                        UpdateCrosswalkRecord,setAlert})(CrossWalkNewDesign);
